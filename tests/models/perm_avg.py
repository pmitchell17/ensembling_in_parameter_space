...
class TestWeightPermutations(unittest.TestCase):
    """Tests functions involving permutation of weight matrices"""
    ...
    def setUp(self):
        ...
        bias = torch.tensor([1, 2, 3], dtype=torch.float32)

        with torch.no_grad():
            model_1.linears[0].weight = nn.Parameter(
                weight_matrix
            )
            model_1.linears[0].bias = nn.Parameter(
                bias
            )
    ...
    def test_get_permuted_layer_weights(self):
        """Tests whether weights and bias of a layer are permuted
        correctly
        """
        perm_matrix = torch.tensor(
            [
        ...
        with patch.object(self.perm_avg, 'get_perm_matrix', return_value=perm_matrix):
            permuted_weights, permuted_bias = \
                self.perm_avg._get_permuted_layer_weights(model_index=1, layer_index=0)

        self.assertEqual(permuted_weights[0].tolist(), [3, 3, 3])
        self.assertEqual(permuted_weights[2].tolist(), [1, 1, 1])
        self.assertEqual(permuted_bias.tolist(), [3, 2, 1])

        def test_get_weights_not_permuted(self):
            """Tests whether raw weights are returned in dict format"""
            unpermuted_weights = self.perm_avg.get_weights(permuted=False)
            
            weights_1_0 = unpermuted_weights[(1, 0)]['weights']
            bias_1_0 = unpermuted_weights[(1, 0)]['bias']

            self.assertEqual(weights_1_0[0].tolist(), [1, 1, 1])
            self.assertEqual(bias_1_0[0].tolist(), [1, 2, 3])
        
        def test_get_weights_permuted(self):
            """Tests whether permuted weights are returned in dict format"""
            perm_matrix_0_0 = torch.eye(size=(3, 3), dtype=torch.float32)
            perm_matrix_0_1 = torch.eye(size=(2, 2), dtype=torch.float32)
            perm_matrix_1_0 = torch.tensor(
                [
                    [0, 0, 1],
                    [0, 1, 0],
                    [1, 0, 0]
            ...
            perm_matrix_1_1 = torch.eye(size=(2, 2), dtype=torch.float32)
            perm_matrices = [
                perm_matrix_0_0,
                perm_matrix_0_1, 
                perm_matrix_1_0,
            ...
            with patch.object(self.perm_avg, 'get_perm_matrix', side_effect=mock_perm_matrices):
                permuted_weights = self.perm_avg.get_weights(permuted=True)
            
            weights_1_0 = permuted_weights[(1, 0)]['weights']
            bias_1_0 = permuted_weights[(1, 0)]['bias']
